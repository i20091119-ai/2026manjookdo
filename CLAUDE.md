...
**단, '유아'는 예외로 학부모/보호자보다 먼저 본다 (2026-09).**
유아는 설문을 스스로 못 써서 응답이 항상 `'유아 학부모'`로만 들어온다.
다른 `'OO 학부모'`(초등학생 학부모 등)처럼 보호자로 세면 자유관람의 유아
헤드카운트가 영원히 0이 된다 — 실제로 이 버그가 있었다. 그래서 `classifyPerson()`
과 `FREE_PART_ENTRIES`는 `'유아'`를 먼저 검사하고, 그다음에 학부모/보호자를 본다.
초/중/고 학부모는 원래대로 보호자로 센다 — 유아만의 예외다.
